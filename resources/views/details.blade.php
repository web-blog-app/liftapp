...
<div class="wrapper">
  ...
  <div class="date">Дата заказа: <span>{{Carbon\Carbon::parse($detail ->updated_at )->format('d-m-Y ')}}</span></div>
  <div class="address">Адрес: <span>{{$detail->address}}</span></div>
  <div class="address">Запчасть: <span>{{$detail->detailName}}</span></div>
  <div class="error">Серийный номер: <span>{{$detail ->number}}</span> </div>
  <div>Текущее состояние: <span>{{$detail ->condition}}</span></div>
   @if( !empty($detail -> notice))
...
<div class="wrapper ">
  ...
  <div class="date">Дата заказа: <span>{{Carbon\Carbon::parse($detail ->updated_at )->format('d-m-Y ')}}</span></div>
  <div class="address">Адрес: <span>{{$detail->address}}</span></div>
   <div >Деталь: <span>{{$detail->detailName}}</span></div>
  <div class="error">Серийный номер: <span>{{$detail ->number}}</span> </div>
  <div>Текущее состояние: <span>{{$detail ->condition}}</span></div>
   @if( !empty($detail -> notice))
  <div class="notice">Примечания: <span>{{$detail ->notice}}</span></div>
